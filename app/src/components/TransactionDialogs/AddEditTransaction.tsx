...
export const AddEditTransaction: React.FC<TransactionDialogData> = (props) => {
  ...
  const createTx = api.transactions.create.useMutation({onSuccess: onMutationSuccess});
  const updateTx = api.transactions.update.useMutation({onSuccess: onMutationSuccess});
  ...
  const handleSubmit = useCallback((values: TransactionFormResult) => {
  ...
  return (
    <>
      ...
      <TransactionForm wallets={wallets} categories={categories} defaultValues={props.transaction} onSubmit={handleSubmit}>
