...
function groupTransactionInCategories(
  type: InOut,
  categories: Category[],
  transactions: TransactionWithJoins[],
) {
  const categoriesWithTransaction: {
    id: string;
    name: string;
    value: number;
    transactions: TransactionWithJoins[];
  ...
  transactions.forEach((t) => {
    ...
    const categoryWithTx = categoriesWithTransaction.find(
      (c) => c.id === t.categoryId,
    );
    ...
    if (categoryWithTx) {
      ...
      categoriesWithTransaction.push({
        id: txCategory.id,
  ...
  categoriesWithTransaction.sort((a, b) => {
  ...
  return categoriesWithTransaction;
}
...
export const TransactionsPerCategoryCard: React.FC<{
  ...
  // const transactions = ctx.transactions.getRange.getData({
  //   from: dateRange.startDate,
  //   to: dateRange.endDate,
  // });

  const [data, sum] = useMemo(() => {
    if (!categories || !transactions) return [undefined, undefined];
    
    const data = groupTransactionInCategories(transactionType, categories, transactions);
    const sum = data.reduce((sum, item) => sum + item.value, 0);

    return [data, sum];
  }, [transactionType, categories, transactions]);
