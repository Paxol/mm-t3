...
export const SavingRate: React.FC<{
  ...
  const data = useMemo(
    () => ({
      in: transactionByCategory.in
        .filter((item) => !(item.atBalance === false))
        .reduce((acc, item) => acc + item.value, 0),
      out: transactionByCategory.out
        .filter((item) => !(item.atBalance === false))
        .reduce((acc, item) => acc + item.value, 0),
    }),
    [transactionByCategory],
    ...
    <Card className="dark:text-white">
      ...
      <div className="flex flex-col overflow-hidden sm:flex-row sm:space-x-4">
        <div className="flex flex-col items-start mt-3">
          <div>
            Entrate: {data.in.toFixed(2)} €{" "}
            <span className="text-slate-700 dark:text-slate-300">
              ({((data.in / sum) * 100).toFixed(0)} %)
            </span>
          </div>
          <div>
            Uscite: {data.out.toFixed(2)} €{" "}
            <span className="text-slate-700 dark:text-slate-300">
              ({((data.out / sum) * 100).toFixed(0)} %)
            </span>
          </div>
